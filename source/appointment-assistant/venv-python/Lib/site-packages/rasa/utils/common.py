...
from rasa.constants import (
    DEFAULT_LOG_LEVEL_LANGFUSE,
    DEFAULT_LOG_LEVEL_LIBRARIES,
    ENV_LOG_LEVEL_KAFKA,
    ENV_LOG_LEVEL_LANGFUSE,
...
def update_langfuse_log_level() -> None:
    """Set the log level for the Langfuse SDK logger.

    When the optional ``langfuse`` package is installed (via the ``monitoring``
    extra) but Langfuse tracing is not configured, the SDK floods the logs with
    ``WARNING`` lines on every turn (e.g. "client initialized without public_key",
    "no active span in current context"). Default to ``ERROR`` so that noise is
    silenced while genuine errors still surface; ``propagate`` is left untouched so
    those errors reach Rasa's root handler. Overridable via ``LOG_LEVEL_LANGFUSE``.

    Note: the ``langfuse`` package sets this logger to ``WARNING`` at import time,
    so ``langfuse_compat`` calls this function right after importing langfuse to
    win the "last setLevel wins" race regardless of import order. This function is
    therefore the single source of truth for the langfuse logger level.
    """
    log_level = os.environ.get(ENV_LOG_LEVEL_LANGFUSE, DEFAULT_LOG_LEVEL_LANGFUSE)

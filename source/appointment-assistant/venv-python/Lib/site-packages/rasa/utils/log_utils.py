    ...
    if not FORCE_JSON_LOGGING and sys.stderr.isatty():
        ...
        processors = shared_processors + [
            HumanConsoleRenderer(
                # `show_locals` defaults to True, which renders every stack
                # frame's local variables. Any class that receives a credential
                # as an argument holds it in a frame local -- tracker stores,
                # event brokers, server JWT config -- so an exception logged
                # with `exc_info` would print it. This is a frame-level leak, so
                # it is fixed here at the renderer rather than per class. Leave
                # it off: no traceback is worth emitting a secret for, and the
                # frame list, exception type and message are all still shown.
        ...
        processors = shared_processors + [
            # `structlog.processors.dict_tracebacks` is
            # `ExceptionRenderer(ExceptionDictTransformer())`, and that
            # transformer defaults to `show_locals=True` -- which is how
            # credentials held in a frame local reached customer log
            # aggregators. Build it explicitly with locals off instead. See the
            # console branch above; both renderers had this default.

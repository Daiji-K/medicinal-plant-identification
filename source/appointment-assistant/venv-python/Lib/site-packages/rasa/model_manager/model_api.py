...
def service_health(request: Request) -> response.HTTPResponse:
    """Liveness endpoint, served at the prefix (`/health`) and at the root (`/`).

    Unauthenticated by design (deployment healthchecks). Keep the body free of
    bot/training identifiers and internal URLs — the full listings live behind
    the authenticated GET /bot and GET /training.
    """
    return json({"status": "ok"})


def internal_blueprint() -> Blueprint:
    """Create a blueprint for the REST API of the model service.

    In the Studio 1.x layout this blueprint is registered at the root and is
    reachable only inside the cluster. In the Studio 2.x layout it is
    registered under the URL prefix, behind the ingress.
    """
...
def external_blueprint() -> Blueprint:
    ...
    bp.add_route(service_health, "/health", name="health")

    return bp


def register_blueprints(app: Sanic, url_prefix: str) -> None:
    """Register the model-service blueprints in the layout for this prefix.

    Studio 1.x (prefix `talk`): the REST API lives at the root,
    reachable only inside the cluster; the prefix serves socket.io and the
    health check, behind the ingress. Studio 2.x (any other prefix): both
    blueprints live under the prefix. `GET /` answers health in both
    layouts, for deployment liveness probes.
    """
    app.blueprint(external_blueprint(), url_prefix=url_prefix)
    if url_prefix.strip("/") == config.STUDIO_V1_PATH_PREFIX:
        app.blueprint(internal_blueprint())
    else:
        app.blueprint(internal_blueprint(), url_prefix=url_prefix)
    app.add_route(service_health, "/", name="root_health")

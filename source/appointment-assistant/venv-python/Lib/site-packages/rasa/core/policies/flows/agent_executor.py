...
async def run_agent(
    ...
    structlogger.debug(
        "flow.agent_executor.run_agent",
        agent_id=agent_name,
        step_id=step.id,
        flow_id=step.flow_id,
        event_info=f"Agent {agent_name} started",
        highlight=True,
    )
    ...
    agent = AgentManager().get_agent(agent_name, protocol_type)

    if agent.is_async():
        return await run_async_agent_step(
...
async def run_agent_sync(
    ...
    final_events = list(initial_events)
    ...
    # Resolve the protocol up front so it can be recorded on the AgentStarted /
    # AgentCompleted events (used by the evaluation event ledger to know whether
    # to expect `mcp_tool` calls) and reused for the agent invocation below.
    protocol_type = get_protocol_type(step, agent_config)
    ...
    if (
        ...
        structlogger.debug(
            "flow.step.run_agent_sync.resuming_interrupted_agent",
            agent_id=agent_name,
            flow_id=step.flow_id,
        )

        # Reinvoke the agent with resume context; events are still submitted.
        final_events.append(AgentResumed(agent_id=agent_name, flow_id=step.flow_id))
        ...
        agent_metadata: Dict[str, Any] = {"protocol": protocol_type.value}
        if agent_config:
        ...
        final_events.append(
            AgentStarted(agent_name, step.flow_id, metadata=agent_metadata)
        )

    structlogger.debug(
        "flow.step.run_agent_sync.agent_input",
        agent_name=agent_name,
        step_id=step.id,
        flow_id=step.flow_id,
    ...
    output.metadata.setdefault(BOT_UTTERANCE_AGENT_NAME_KEY, agent_name)

    structlogger.debug(
        "flow.step.run_agent_sync.agent_response",
        agent_name=agent_name,
        step_id=step.id,
        flow_id=step.flow_id,
    ...
    structlogger.debug(
        "flow.step.run_agent_sync.agent_finished",
        agent_name=agent_name,
        flow_id=step.flow_id,
        agent_state=str(output.status),
        agent_response_message=output.response_message,
        highlight=True,
        event_info=f"Agent {agent_name} finished",
    )
    ...
    if output.status == AgentStatus.INPUT_REQUIRED:
        ...
        return _handle_agent_completed(output, final_events, stack, step, protocol_type)
...
def _processing_deadline_expired(agent_stack_frame: AgentStackFrame) -> bool:
    deadline = agent_stack_frame.processing_deadline
    return deadline is not None and deadline <= time.time()


def _handle_expired_processing_agent(
    ...
    structlogger.warning(
        "flow.step.run_async_agent_step.processing_timeout",
        agent_id=agent_name,
        flow_id=step.flow_id,
        task_id=agent_stack_frame.agent_task_id,
        processing_deadline=agent_stack_frame.processing_deadline,
    )
    protocol_type = get_protocol_type(
        step,
        Configuration.get_instance().available_agents.get_agent_config(agent_name),
    )
    return _handle_agent_fatal_error(
        AgentOutput(
            id=agent_name,
            ...
            metadata=agent_stack_frame.metadata,
        ),
        final_events,
        stack,
        step,
...
async def run_async_agent_step(
    initial_events: List[Event],
    stack: DialogueStack,
    step: CallFlowStep,
    tracker: DialogueStateTracker,
    ...
    structlogger.debug(
        "flow.step.run_async_agent_step",
        agent_id=agent_name,
        step_id=step.id,
        flow_id=step.flow_id,
        event_info=f"Agent {agent_name} started",
    ...
    final_events = list(initial_events)
    agent_stack_frame = stack.find_agent_stack_frame_by_agent(agent_id=agent_name)

    if agent_stack_frame and agent_stack_frame.state == AgentState.PROCESSING:
        if _processing_deadline_expired(agent_stack_frame):
            return _handle_expired_processing_agent(
                agent_stack_frame,
                final_events,
                stack,
                step,
        ...
        structlogger.debug(
            "flow.step.run_async_agent_step.handling_message_on_processing_agent",
            agent_id=agent_name,
            flow_id=step.flow_id,
            message=agent_stack_frame.metadata.get(ACTION_METADATA_MESSAGE_KEY, {}).get(
                ACTION_METADATA_TEXT_KEY, None
            )
        ...
        return _handle_agent_processing_update(final_events, stack, step)

    if agent_stack_frame and agent_stack_frame.state == AgentState.TERMINAL:
        structlogger.debug(
            "flow.step.run_async_agent_step.handling_terminal_agent",
            agent_id=agent_name,
            flow_id=step.flow_id,
            message=agent_stack_frame.metadata.get(ACTION_METADATA_MESSAGE_KEY, {}).get(
                ACTION_METADATA_TEXT_KEY, None
        ...
        message = (
            agent_stack_frame.metadata.get(ACTION_METADATA_MESSAGE_KEY, {}).get(
                ACTION_METADATA_TEXT_KEY, None
            )
            if agent_stack_frame.metadata
            else None
        ...
        return _handle_agent_fatal_error(
            AgentOutput(
                id=agent_name,
                status=AgentStatus.FATAL_ERROR,
                error_message=message or "Agent task ended unsuccessfully.",
                metadata=agent_stack_frame.metadata,
            ),
            final_events,
            stack,
    ...
    if (
        agent_stack_frame
        and agent_stack_frame.state
        == AgentState.WAITING_FOR_INPUT_FROM_PUSH_NOTIFICATION
    ):
        structlogger.debug(
            "flow.step.run_async_agent_step.handling_message_on_agent_waiting_for_input",
            agent_id=agent_name,
            flow_id=step.flow_id,
            message=agent_stack_frame.metadata.get(ACTION_METADATA_MESSAGE_KEY, {}).get(
                ACTION_METADATA_TEXT_KEY, None
    ...
    if agent_stack_frame and agent_stack_frame.state == AgentState.COMPLETED:
        structlogger.debug(
            "flow.step.run_async_agent_step.handle_completed",
            agent_name=agent_name,
            step_id=step.id,
            flow_id=step.flow_id,
            message=agent_stack_frame.metadata.get(ACTION_METADATA_MESSAGE_KEY, {}).get(
        ...
        remove_agent_stack_frame(stack, agent_name)
        agent_completed_event = AgentCompleted(
            agent_id=agent_name, flow_id=step.flow_id
        )
        ...
        if message:
            return PauseFlowReturnPrediction(
                _create_send_text_prediction(message, final_events, {})
            )
    ...
    if (
        ...
        structlogger.debug(
            "flow.step.run_async_agent_step.resuming_interrupted_agent",
            agent_id=agent_name,
            flow_id=step.flow_id,
        )
        ...
        # add the AgentStarted event to the list of final events
        final_events.append(AgentStarted(agent_name, step.flow_id))

    structlogger.debug(
        "flow.step.run_async_agent_step.agent_input",
        agent_name=agent_name,
        step_id=step.id,
        flow_id=step.flow_id,
        agent_input=agent_input.model_dump(),
    ...
    # send the input to the agent and wait for a response
    protocol_type = get_protocol_type(
        step, Configuration.get_instance().available_agents.get_agent_config(agent_name)
    )
    output: AgentOutput = await _call_async_agent_with_retry(
        protocol_type=protocol_type,
        agent_input=agent_input,
        max_retries=MAX_AGENT_RETRIES,
        output_channel=output_channel,
    ...
    structlogger.debug(
        "flow.step.run_async_agent_step.agent_response",
        agent_name=agent_name,
        step_id=step.id,
        flow_id=step.flow_id,
        agent_response=output.model_dump(),
    ...
    structlogger.debug(
        "flow.step.run_async_agent_step.agent_finished",
        agent_name=agent_name,
        flow_id=step.flow_id,
        agent_state=str(output.status),
        agent_response_message=output.response_message,
    ...
    elif output.status == AgentStatus.SUBMITTED:
        return _handle_submitted(output, final_events, stack, step)
    else:
        return _handle_agent_unknown_status(
            output, final_events, stack, step, flows, tracker
        )
...
async def _call_agent_with_retry(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # We don't have a valid agent response at this time to act based
...
async def _call_async_agent_with_retry(
    protocol_type: ProtocolType,
    agent_input: AgentInput,
    max_retries: int,
    output_channel: Optional[OutputChannel] = None,
    ...
    for attempt in range(max_retries):
        ...
        try:
            agent_response: AgentOutput = await AgentManager().run_async_agent(
                protocol_type=protocol_type,
                agent_input=agent_input,
                output_channel=output_channel,
                cancellation_token=cancellation_token,
...
def _handle_agent_unknown_status(
    ...
    Args:
        ...
        protocol_type: The protocol type of the agent, if known
    ...
    remove_agent_stack_frame(stack, agent_name)
    final_events.append(AgentCancelled(agent_id=agent_name, flow_id=step.flow_id))
...
def _handle_agent_processing_update(
    final_events: List[Event],
    stack: DialogueStack,
    step: CallFlowStep,
) -> FlowStepResult:
    ...
    defaults = _build_default_agent_message_metadata(
        step, BOT_UTTERANCE_AGENT_MESSAGE_TYPE_INTERMEDIATE_MESSAGE
    )

    top_stack_frame = stack.top()
    agent_name = step.call

    if not isinstance(top_stack_frame, AgentStackFrame):
        structlogger.debug(
            "flow_executor.handle_agent_processing_update_error",
            agent_name=agent_name,
            flow_id=step.flow_id,
        )
        raise RuntimeError(
            "handle_agent_processing_update_error."
            "top_stack_frame.not_agent_stack_frame",
        )

    action_prediction = _create_action_prediction(
        ACTION_AGENT_SEND_TEXT_UPDATE,
        top_stack_frame.metadata.get(ACTION_METADATA_MESSAGE_KEY, {}).get(
            ACTION_METADATA_TEXT_KEY, None
        )
        if top_stack_frame.metadata
        else None,
        final_events,
        defaults,
    )
...
def _handle_agent_input_required(
    ...
    if isinstance(top_stack_frame, AgentStackFrame):
        ...
        top_stack_frame.agent_id = agent_name
...
def _handle_submitted(
    ...
    top_stack_frame = stack.top()
    agent_name = step.call
    task_id = output.task_id or output.metadata.get(A2A_AGENT_TASK_ID_KEY, "")
    context_id = output.context_id or output.metadata.get(A2A_AGENT_CONTEXT_ID_KEY, "")
    processing_deadline = time.time() + ASYNC_AGENT_PROCESSING_TIMEOUT_SECONDS
    # update the agent stack frame if it is already on the stack
    # otherwise push a new one
    if isinstance(top_stack_frame, AgentStackFrame):
        top_stack_frame.state = AgentState.PROCESSING
        top_stack_frame.metadata = output.metadata
        top_stack_frame.step_id = step.id
        top_stack_frame.agent_id = agent_name
        top_stack_frame.flow_id = step.flow_id
        top_stack_frame.agent_task_id = task_id
        top_stack_frame.context_id = context_id
        top_stack_frame.processing_deadline = processing_deadline
    else:
        stack.push(
            AgentStackFrame(
                flow_id=step.flow_id,
    ...
    action_prediction = _create_agent_submitted_prediction(
        output.response_message, final_events, output.metadata
    )
    return PauseFlowReturnPrediction(action_prediction)
...
def _handle_agent_completed(
    ...
    Args:
        ...
        stack: The dialogue stack
        step: The flow step that called the agent
        protocol_type: The resolved agent protocol, recorded on the
            ``AgentCompleted`` event metadata (used by the evaluation event
            ledger to decide whether to expect `mcp_tool` calls).
    ...
    agent_completed_event = AgentCompleted(
        ...
        metadata={"protocol": protocol_type.value} if protocol_type else None,
    )
    final_events.append(agent_completed_event)
...
def _create_agent_submitted_prediction(
    message: Optional[str],
    events: Optional[List[Event]],
    metadata: Optional[Dict[str, Any]] = None,
) -> FlowActionPrediction:
    ...
    This function creates a prediction that will pause the flow and wait for user input.
    """
    return _create_action_prediction(ACTION_LISTEN_NAME, message, events, metadata)
...
def _prepare_agent_input(
    ...
    _update_agent_input_metadata_with_events(
        agent_input_metadata, agent_name, step.flow_id, tracker
    )
    ...
    if agent_stack_frame and agent_stack_frame.agent_task_id:
        agent_input_metadata[A2A_AGENT_TASK_ID_KEY] = agent_stack_frame.agent_task_id
    agent_input_metadata[A2A_AGENT_CONTEXT_ID_KEY] = (
        agent_stack_frame.context_id
        if agent_stack_frame and agent_stack_frame.context_id
        else tracker.sender_id
    )
        ...
        if slot_definition:
            filtered_slots.append(
                AgentInputSlot(
                    ...
                    mappings=[
                        mapping.as_dict() for mapping in slot_definition.mappings
                    ],
                )
            )

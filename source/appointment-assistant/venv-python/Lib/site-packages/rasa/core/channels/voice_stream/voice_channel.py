...
from rasa.core.channels.voice_stream.call_recording.recording import (
    CallRecordingSession,
    create_call_recording_session,
)
...
# Grace period for the agent task to drain remaining events
# (notably SessionEndedInputEvent) before being cancelled on shutdown.
AGENT_SHUTDOWN_GRACE_PERIOD_SECONDS = 30.0
...
@dataclass
class TextInputAction(VoiceChannelAction):
    text: str
...
class VoiceOutputChannel(OutputChannel):
    def __init__(
        ...
        self.recording_session: Optional[CallRecordingSession] = None
    ...
    async def send_image_url(
        self, recipient_id: str, image: str, **kwargs: Any
    ) -> None:
        logger.warning(
            "voice_channel.image_suppressed",
            reason="Images cannot be sent via voice channels.",
        )

    async def send_attachment(
        self, recipient_id: str, attachment: str, **kwargs: Any
    ) -> None:
        logger.warning(
            "voice_channel.attachment_suppressed",
            reason="Attachments cannot be sent via voice channels.",
        )

    async def send_custom_json(
        self, recipient_id: str, json_message: Dict[str, Any], **kwargs: Any
    ) -> None:
        """Suppress custom JSON on voice channels that cannot transport it.

        Concrete channels with a data-frame protocol can override this method.
        The voice default must not delegate to ``OutputChannel``, because that
        implementation serializes the payload as text and sends it to TTS.
        """
        logger.warning(
            "voice_channel.custom_json_suppressed",
            reason="Custom JSON cannot be sent via this voice channel.",
        )

    def _track_rasa_processing_latency(self) -> None:
        """Track Rasa processing latency for the current bot message.

        Only meaningful for the first bot message of a turn, where the window is opened
        on the final ASR transcript. Subsequent bot messages in the same turn produce no
        measurement.
        """
        if not call_state.rasa_processing_start_time:
            logger.debug(
                "voice_channel.rasa_processing_latency.skipped",
                reason="rasa_processing_start_time is not set",
            )
            return
        ...
        # Close the window; follow-up bot messages in the same turn will not have a
        # measurement since there is no reliable signal for when Rasa starts processing
        # the next action.
        call_state.rasa_processing_start_time = None
        logger.debug("voice_channel.rasa_processing_window.closed")

    def _reset_tts_timing(self) -> None:
        """Reset TTS timing so stale values from the previous response never leak."""
        call_state.tts_start_time = time.time()
        call_state.tts_first_byte_latency_ms = None
        call_state.tts_complete_latency_ms = None
        call_state.tts_read_from_cache = False
        logger.debug(
            "voice_channel.tts_timing_reset",
            tts_start_time=call_state.tts_start_time,
        )

    def _track_tts_first_byte_latency(self) -> None:
        """Track and log TTS first byte latency."""
        if not call_state.tts_start_time:
            logger.debug(
                "voice_channel.tts_first_byte_latency.skipped",
                reason="tts_start_time is not set",
            )
            return
    ...
    def _track_tts_complete_latency(self) -> None:
        ...
        No-op if already recorded (e.g. captured at barge-in time).
        """
        if (
            not call_state.tts_start_time
            or call_state.tts_complete_latency_ms is not None
        ):
            logger.debug(
                "voice_channel.tts_complete_latency.skipped",
                tts_start_time=call_state.tts_start_time,
    ...
    async def _stream_audio_to_channel(
        self,
        recipient_id: str,
        audio_stream: AsyncIterator[RasaAudioBytes],
        track_latency: bool = True,
    ) -> Optional[RasaAudioBytes]:
        """Send audio from an async iterator to the channel.

        This function does a lot of things,
        - Tracks TTS first byte latency. The latency is recorded on the very first
          audio chunk produced by the TTS engine, even if we end up suppressing the send
          (e.g. user barge-in) — so a TTSFinished event still carries a meaningful
          first-byte metric in those cases. Skipped when ``track_latency`` is False
          (e.g. silence for pacing).
        ...
        async for audio_chunk in audio_stream:
            ...
            # Track first byte BEFORE the barge-in check so the metric reflects when the
            # TTS produced its first audio, even when we drop the bytes.
            if track_latency and not first_byte_received:
                self._track_tts_first_byte_latency()
                first_byte_received = True

            if self.tts_engine.stop_streaming_output_audio_chunks:
                if not suppression_logged:
                    # Capture partial duration at the moment we start dropping bytes
                    if track_latency:
                        self._track_tts_complete_latency()
                    self.stream_interrupted = True
                    logger.debug(
                        "voice_channel.audio_streaming_suppressed",
                        recipient_id=recipient_id,
                        reason=(
                            "user is speaking; TTS audio dropped but timings"
                            " still recorded for TTSFinished"
                        ),
                    )
    ...
    async def apply_pacing_delay(self, recipient_id: str, seconds: float) -> None:
        ...
        # Silence is not TTS output, so don't let it pollute the first-byte latency
        # metric (would otherwise record the pacing wait as TTS latency).
        await self._stream_audio_to_channel(
            recipient_id, self.chunk_audio(silence), track_latency=False
        )
    ...
    async def _start_tts_message(self, recipient_id: str) -> None:
        """Prepare per-message TTS state at the start of a bot message.

        Captures Rasa processing latency before pacing (so the pacing wait does not
        inflate it), applies pacing silence, then resets timing and the interruption
        flag so they reflect only the actual TTS that is about to be produced — not the
        silence pad or stale state from a previous message.

        Why the interruption flag is reset here:
            The flag answers "was this spoken message cut short by the user?"
            Pacing silence is not part of any spoken message, so a barge-in detected
            during pacing must not taint the next message.

            Example — bot sends two messages back-to-back:
                1. "Got it."                                  → played in full
                2. [500ms pacing silence; user briefly says "mhm yeah sure" then stops]
                3. "Your appointment is on Monday at 3pm."    → played in full

            Message 2 was delivered without interruption. Without this reset the brief
            pacing-time speech would mark message 2's TTSFinished as interrupted=True,
            polluting analytics and dialog decisions.

            A real barge-in during step 3's audio is still captured normally.

        Args:
            recipient_id: The recipient ID, forwarded to the pacing helper.
        """
        self._track_rasa_processing_latency()
        await self._apply_min_delay_between_messages(recipient_id)
        # Pacing silence is not part of the spoken response — reset timing and
        # stream_interrupted so they cover only the actual TTS.
        self._reset_tts_timing()
        self.stream_interrupted = False
        logger.debug(
            "voice_channel.tts_message_started",
            recipient_id=recipient_id,
        )

    async def _finish_tts_message(
        self,
        recipient_id: str,
        step_type: StepType,
    ) -> None:
        """Finalize per-message TTS state at the end of a bot message.

        Records TTS completion latency, sends the end marker, marks the end-of-message
        timestamp so the next message can pace correctly, and logs the per-message
        timing summary. Counterpart to `_start_tts_message`.

        Args:
            recipient_id: The recipient ID, used for the end marker and log entry.
            step_type: The step type to attach to the end marker.
        """
        self._track_tts_complete_latency()
        ...
        logger.debug(
            "voice_channel.tts_message_ended",
            recipient_id=recipient_id,
            tts_first_byte_latency_ms=call_state.tts_first_byte_latency_ms,
            tts_complete_latency_ms=call_state.tts_complete_latency_ms,
            interrupted=self.stream_interrupted,
        )
    ...
    async def send_response_chunk_start(
        ...
        """
        await self._start_tts_message(recipient_id)
        ...
        logger.debug(
            "voice_channel.start_streaming_response",
            recipient_id=recipient_id,
            streaming_input=self.tts_engine.streaming_input,
        )
    ...
    async def send_response_chunk_end(self, recipient_id: Text, **kwargs: Any) -> None:
        ...
        # Proactively detect interruption after audio task completes.
        # stop_streaming_output_audio_chunks is set True when:
        #   - stop_streaming() is called from receive_asr_events (barge-in), OR
        #   - user was already speaking when audio streaming started.
        # Both cases mean the user interrupted before or during audio playback.
        # This ensures executors that check stream_interrupted after chunk_end /
        # stream_end can reliably enter drain mode even when all LLM chunks arrived
        # before the user interrupted (the common fast-LLM / slow-TTS scenario).
        if self.tts_engine.stop_streaming_output_audio_chunks:
            self.stream_interrupted = True
            logger.debug(
                "voice_channel.send_response_chunk_end.stream_interrupted",
                recipient_id=recipient_id,
            )

        self.tts_engine.stream_state = StreamState.NO_STREAMING
        await self._finish_tts_message(
            recipient_id,
            step_type=StepType.COLLECT
            if kwargs.get(STEP_TYPE_METADATA_KEY) == COLLECT_STEP_TYPE
            else StepType.REGULAR_UTTER,
    ...
    async def send_response(
        ...
        """Send the response and report TTS timing back to the caller.

        Delegates the actual rendering to the base channel implementation, then
        snapshots the TTS timing captured in `call_state` during synthesis and packages
        it as a `TTSFinished` event in an `OutputDeliveryResult`. This is the signal the
        processor uses to attach TTS metrics to the tracker.

        Args:
            recipient_id: The recipient ID.
            message: The outgoing response payload (text and any channel-specific
                fields) handled by the base implementation.

        Returns:
            - `OutputDeliveryResult` with a `TTSFinished` event carrying
            `tts_time_to_first_byte_ms`, `read_from_cache`, and optionally
            `tts_total_time_ms` under `voice_latency`, plus `interrupted`.
            - `None` when no TTS timing was captured
        """
        await super().send_response(recipient_id, message)

        ttfb_ms = call_state.tts_first_byte_latency_ms
        total_ms = call_state.tts_complete_latency_ms
        read_from_cache = call_state.tts_read_from_cache
        interrupted = self.stream_interrupted

        if ttfb_ms is None:
            logger.debug(
                "voice_channel.tts_finished_no_metadata",
                recipient_id=recipient_id,
            )
            return None

        voice_latency: Dict[str, Any] = {"tts_time_to_first_byte_ms": ttfb_ms}
        if total_ms is not None:
            voice_latency["tts_total_time_ms"] = total_ms
        voice_latency["read_from_cache"] = read_from_cache
        ...
        logger.debug(
            "voice_channel.tts_finished_event",
            recipient_id=recipient_id,
            tts_time_to_first_byte_ms=ttfb_ms,
            tts_total_time_ms=total_ms,
    ...
    async def send_text_message(
        ...
        logger.debug("voice_channel.send_text_message", text=text)
        if (
        ...
        call_state.tts_read_from_cache = cached_audio_bytes is not None
        logger.debug(
...
class VoiceInputChannel(InputChannel):
    ...
    # True when the client acks playback markers (so we can wait for real playback to
    # complete)
    tracks_audio_playback = True
    ...
    def build_turn_metadata(self, call_parameters: CallParameters) -> Dict[str, Any]:
        """Builds the metadata stamped onto a user turn.

        Every voice turn carries the call parameters collected at connect time
        so custom actions can read them from `tracker.latest_message["metadata"]`.

        Args:
            call_parameters: The call parameters collected when the call began.

        Returns:
            The call parameters as a dict.
        """
        return asdict(call_parameters)
    ...
    async def receive_asr_events(
        ...
        call_parameters: CallParameters,
        input_queue: ConversationQueue[VoiceInputEvent],
        recording_session: Optional[CallRecordingSession] = None,
    ) -> None:
        ...
        async for event in asr_engine.stream_asr_events():
            ...
            if should_interrupt:
                ...
                # Mark that the bot playback has stopped so the recording session can
                # stop recording the bot audio.
                # Genesys does not support interrupting bot playback so recording
                # might contain parts of bot responses that were interrupted by the user
                # and may not have been completely played on the call.
                if recording_session is not None:
                    recording_session.mark_bot_playback_stopped()
                await input_queue.put(BargeInInputEvent())
                # monitor_internal_queue clears this asynchronously via
                # BotStoppedSpeaking. Clear it now so burst events don't re-trigger
                # barge-in.
                call_state.is_bot_speaking = False
                # Client drops its audio queue on interrupt and won't ack these
                # markers; clear them so silence monitoring can re-arm later.
                call_state.audio_markers.clear()
    ...
    def _initialize_call_state(self, model_metadata: Optional["ModelMetadata"]) -> None:
        call_state_ = CallState(
            ...
            interruption_config=self.interruption_config,
            tracks_audio_playback=self.tracks_audio_playback,
        )
    ...
    async def run_audio_streaming(
        ...
        recording_session = create_call_recording_session(
            config=agent.recording_config,
            conversation_id=sender_id,
            call_parameters=call_parameters,
            audio_format=self.audio_format,
            event_broker=agent.event_broker,
        )
        ...
        output_channel.recording_session = recording_session
        language_plugin = VoiceLanguageChangePlugin(sender_id, asr_engine, tts_engine)

        # Concurrent streaming. `agent_task` is kept separate from `io_tasks` so
        # shutdown can await it (not only cancel it) and let session-end events finish.
        agent_task: Optional[asyncio.Task[Any]] = None
        io_tasks: List[asyncio.Task[Any]] = []
        ...
        try:
            ...
            await self.start_session(
                input_queue,
                call_parameters,
                recording_session.started_metadata if recording_session else None,
            )

            io_tasks = [
                asyncio.create_task(
                    self._consume_audio_bytes(
                        channel_websocket,
                        asr_engine,
                        input_queue,
                        call_parameters,
                ...
                asyncio.create_task(
                    self.asr_keep_alive_task(asr_engine), name="asr_keep_alive"
                ),
            ]
            # Include agent_task so a crash in the agent also unblocks the
            # wait and triggers cleanup.
            ...
            for task in done:
                ...
                if exception is not None:
                    logger.error(
                        "voice_channel.streaming_task_failed",
                        call_id=call_parameters.call_id,
                        task=task_name,
                        exception_type=type(exception).__name__,
                    )
                elif task_name != "caller_audio" or task.cancelled():
                    logger.warning(
                        "voice_channel.streaming_task_ended_unexpectedly",
                        call_id=call_parameters.call_id,
                        task=task_name,
                    )
        finally:
            # Marked before the shutdown grace period so the recording ends at the
            # moment the call dropped, not when the agent finishes shutting down.
            if recording_session is not None:
                recording_session.mark_call_ended()

            # Cancel I/O tasks first so no new events are enqueued, but the
            # SessionEndedInputEvent already in the queue is left for the agent.
            for task in io_tasks:
                task.cancel()
            if io_tasks:
                await asyncio.gather(*io_tasks, return_exceptions=True)

            # Give the agent a grace period to finish pending work (e.g. session end)
            # before closing ASR/TTS. Cancels the task if it hangs.
            if agent_task is not None:
                await self._await_agent_shutdown(agent_task, call_parameters)
            ...
            if recording_session is not None:
                recording_session.finalize()
            call_state.stop_all()

    async def _consume_audio_bytes(
        self,
        channel_websocket: Websocket,
        asr_engine: ASREngine,
        input_queue: ConversationQueue[VoiceInputEvent],
        ...
        """Read audio frames from the websocket and route them to ASR or the queue.

        Runs until an explicit disconnect message is received or the websocket is closed
        by the remote end, then enqueues a SessionEndedInputEvent via handle_disconnect
        so the agent can shut down cleanly.
        """
        ...
        try:
            async for message in channel_websocket:
                ...
                if isinstance(channel_action, NewAudioAction):
                    if recording_session is not None:
                        recording_session.record_user_audio(channel_action.audio_bytes)
                    try:
                        ...
                        # Defense in depth for ASR subclasses that don't swallow
                        # ConnectionClosed* internally (the base ASREngine does).
                        # Dropping the chunk avoids tearing down the call task.
                        logger.debug(
                            "voice_channel.audio_streaming."
                            "asr_connection_closed_drop_chunk",
                            error=str(e),
                        )
                        continue
                if isinstance(channel_action, TextInputAction):
                    self._mark_rasa_processing_started()
                    await input_queue.put(
                        TextInputEvent(
                            text=channel_action.text,
                            metadata=asdict(call_parameters),
                        )
                    )
                elif isinstance(channel_action, DTMFInputAction):
                    ...
                    await self.handle_disconnect(input_queue, call_parameters)
                    break
    ...
    async def _await_agent_shutdown(
        self,
        agent_task: asyncio.Task[Any],
        call_parameters: CallParameters,
    ) -> None:
        """Wait for the agent task to shut down after I/O has stopped.

        Bounded by `AGENT_SHUTDOWN_GRACE_PERIOD_SECONDS`. On timeout, the task is
        cancelled. Exceptions from the agent are logged and not re-raised so channel
        cleanup still runs.
        """
        if agent_task.done():
            if agent_task.cancelled():
                return
            if exc := agent_task.exception():
                logger.error(
                    "voice_channel.agent_task_failed",
                    event_info=(
                        "Agent task raised an exception before shutdown. "
                        "SessionEnded may not have been processed."
                    ),
                    call_id=call_parameters.call_id,
                    error=str(exc),
                )
        ...
        try:
            # wait_for cancels agent_task on timeout and re-raises TimeoutError.
            await asyncio.wait_for(
                agent_task,
                timeout=AGENT_SHUTDOWN_GRACE_PERIOD_SECONDS,
            )
            logger.debug(
                "voice_channel.agent_task_shutdown_completed",
                event_info=(
                    "Agent task finished within the "
                    f"{AGENT_SHUTDOWN_GRACE_PERIOD_SECONDS} seconds grace period and "
                    "was not cancelled."
                ),
                call_id=call_parameters.call_id,
            )
            ...
            logger.error(
                "voice_channel.agent_task_shutdown_timeout",
                event_info=(
                    "Agent task did not finish within the "
                    f"{AGENT_SHUTDOWN_GRACE_PERIOD_SECONDS} seconds grace period and "
                    "was cancelled. SessionEnded may not have been processed."
                ),
                call_id=call_parameters.call_id,
                timeout_seconds=AGENT_SHUTDOWN_GRACE_PERIOD_SECONDS,
            )
        except Exception as e:
            logger.error(
                "voice_channel.agent_task_failed",
                event_info=(
                    "Agent task raised an exception during shutdown. "
                    "SessionEnded may not have been processed."
                ),
                call_id=call_parameters.call_id,
                error=str(e),
    ...
    def _mark_rasa_processing_started(self) -> None:
        """Open the Rasa-processing window for the first bot message of a turn."""
        call_state.rasa_processing_start_time = time.time()
        logger.debug(
            "voice_channel.rasa_processing_window.opened",
            rasa_processing_start_time=call_state.rasa_processing_start_time,
        )
    ...
    async def handle_asr_event(
        ...
        if isinstance(asr_event, NewTranscript) and asr_event.text:
            ...
            # Track ASR and open the Rasa-processing window for the first bot
            # message of this turn.
            ...
            event = SilenceDetectedInputEvent(
                metadata=self.build_turn_metadata(call_parameters)
            )
            await input_queue.put(event)
    ...
    async def submit_dtmf_input(
        ...
        dtmf_slot = call_state.dtmf_slot
        call_state.is_collecting_dtmf = False
        call_state.dtmf_buffer = ""
        call_state.dtmf_slot = None

        event = DTMFInputEvent(text=dtmf_input, slot_name=dtmf_slot)
        await input_queue.put(event)

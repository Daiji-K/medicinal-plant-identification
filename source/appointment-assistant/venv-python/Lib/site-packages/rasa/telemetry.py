...
from rasa.utils.secret_redaction import MODEL_CONFIG_NOISE_KEYS, redact_config_secrets
...
def track_model_training(
    ...
    Args:
        ...
        role: Role of the caller that initiated the training. Reported verbatim in
            the telemetry event.
        action: Action that initiated the training. Reported verbatim in the
            telemetry event.
    """
    ...
    # These three sections are customer-authored config that can contain
    # hardcoded credentials, so they are redacted before being reported. They are
    # read once here because the hardcoded-secret check below needs the raw
    # values, which redaction discards.
    pipeline = config.get(CONFIG_PIPELINE_KEY)
    policies = config.get(CONFIG_POLICIES_KEY)
    configuration = rasa.core.config.configuration.Configuration.get_instance()
    model_groups = configuration.endpoints.model_groups
    ...
    tracking_data = {
        ...
        "pipeline": json.dumps(redact_config_secrets(pipeline)),
        "policies": json.dumps(redact_config_secrets(policies)),
        "model_groups": json.dumps(
            redact_config_secrets(model_groups, drop_keys=MODEL_CONFIG_NOISE_KEYS)
        ),
...
def _collect_agent_configuration(flows: FlowsList) -> Dict[str, Any]:
    ...
    # Dump agents as json, excluding any values that are None.
    # It carries customer credentials (sub-agent `auth` blocks),
    # so every dump is redacted before it reaches the payload.
    if agents:
        agent_data["agents"] = [
            {
                agent_name: redact_config_secrets(
                    agent_info.model_dump(exclude_none=True)
                )
            }

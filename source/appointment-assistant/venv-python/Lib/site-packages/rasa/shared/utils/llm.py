...
def _get_enterprise_search_prompt(
    config: Dict[Text, Any],
    model_groups: Optional[List[Dict[str, Any]]] = None,
) -> Text:
    """Returns the default enterprise search prompt for the policy config.

    Args:
        config: The config.yml file data.
        model_groups: Model groups from endpoints.yml. Required when
            Configuration endpoints are empty (Studio / model-service).

    Returns:
        The resolved default enterprise search prompt template.
    """
    ...
    return EnterpriseSearchPolicy.get_system_default_prompt_based_on_config(
        enterprise_search_config, model_groups=model_groups
    )
...
def get_system_default_prompts(
    ...
    return SystemPrompts(
        ...
        enterprise_search=_get_enterprise_search_prompt(config, model_groups),
